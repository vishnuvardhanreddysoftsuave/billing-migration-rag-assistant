...
# --------------------------------------------------------------------------
# Week 5: reading traces, open coding, and a ranked error taxonomy
# --------------------------------------------------------------------------


def write_week5_artifacts(
    trace_set,
    coding: List[Dict[str, Any]],
    taxonomy: List[Any],
    pool_category_counts: Dict[str, int],
    out_dir: Path,
    repo_root: Path,
) -> Path:
    out_dir.mkdir(parents=True, exist_ok=True)
    results_path = repo_root / "results_week5.md"
    results_path.write_text(
        _week5_markdown(trace_set, coding, taxonomy, pool_category_counts, repo_root), encoding="utf-8"
    )
    return results_path
...
def _week5_markdown(
    ...
    cfg = trace_set.config_summary
    lines: List[str] = []
    add = lines.append

    add("# Week 5 Task Set A — Error Analysis: Results")
    add("")
    add(f"*Generated by `python rag.py eval-errors` at {trace_set.generated_at}. The 20 traces below")
    add("are a real, seeded random sample run through the pipeline exactly as it ships today; the")
    add("open-coding notes and the taxonomy grouping are hand-written in `eval/week5_open_coding.yaml`,")
    add("cross-checked against every trace by `validate_open_coding` before this report is written.*")
    add("")
    add("| Setting | Value |")
    add("|---|---|")
    add(f"| Chunking strategy | `{cfg['strategy']}` |")
    add(f"| Retrieval mode | `{cfg['retrieval_mode']}` (the shipped default, not pinned for this week) |")
    add(f"| Generation backend | `{cfg['generation_backend']}` |")
    add(f"| Index namespace | `{cfg['namespace']}` |")
    add(f"| Sample size / pool size / seed | {trace_set.n_traces} / {trace_set.pool_size} / {trace_set.seed} |")
    add("")

    # -- 0. methodology ---------------------------------------------------
    add("## 0. Sampling: random, not curated")
    add("")
    add("`eval/week5_questions.yaml` is a pool of 84 candidate questions meant to stand in for real")
    add("traffic, not for a nice demo: one item per troubleshooting-table error code across all six")
    add("new articles (51, none hand-picked), plus prose-fact questions over both the new and legacy")
    add("articles, colloquial paraphrases, genuinely out-of-corpus questions, and deliberately hard")
    add("ambiguous or multi-part questions. The 20 traces below are")
    add("`random.Random(5).sample(pool, 20)` — a seed fixed *before* the sampler was ever run and never")
    add("re-rolled after seeing the result. Pool composition vs. what got drawn:")
    add("")
    add("| Category | In pool | In this sample |")
    add("|---|---|---|")
    sampled_counts: Dict[str, int] = {}
    for r in trace_set.records:
        sampled_counts[r.category] = sampled_counts.get(r.category, 0) + 1
    for category in pool_category_counts:
        add(f"| `{category}` | {pool_category_counts[category]} | {sampled_counts.get(category, 0)} |")
    add("")

    # -- 1. per-trace table -------------------------------------------------
    add("## 1. Every trace, read and coded")
    add("")
    add("One honest note per trace, written from that trace alone before any grouping happened —")
    add("`eval/week5_open_coding.yaml` carries the full text; this table is the compressed view.")
    add("")
    add("| id | category | outcome | severity | note |")
    add("|---|---|---|---|---|")
    by_id = {row["trace_id"]: row for row in coding}
    for r in trace_set.records:
        row = by_id[r.trace_id]
        note = " ".join(row["note"].split())
        add(f"| `{r.trace_id}` | {r.category} | {row['outcome']} | {row['severity']} | {note} |")
    add("")

    n_no_issue = sum(1 for row in coding if row["severity"] == 0)
    add(
        f"**{n_no_issue}/{trace_set.n_traces} traces had no issue** — {trace_set.n_answered} answered,"
        f" {trace_set.n_refused} refused, out of {trace_set.n_traces} total."
    )
    add("")

    # -- 2. taxonomy ----------------------------------------------------
    add("## 2. Named problems, ranked by frequency x severity")
    add("")
    add("Severity: 3 = severe (a good answer existed and the user did not get it, or got a wrong one")
    add("with no signal it might be wrong), 2 = moderate (defensible but a better system would do")
    add("more), 1 = cosmetic (correct outcome, quality nit).")
    add("")
    add("| Rank | Problem | Count | Mean severity | Score (count x severity) | Example traces |")
    add("|---|---|---|---|---|---|")
    for i, row in enumerate(taxonomy, start=1):
        examples = ", ".join(f"`{t}`" for t in row.trace_ids[:6])
        if len(row.trace_ids) > 6:
            examples += f", +{len(row.trace_ids) - 6} more"
        add(f"| {i} | **{row.name}** | {row.count} | {row.mean_severity:.1f} | {row.score:.1f} | {examples} |")
    add("")
    for row in taxonomy:
        add(f"**{row.name}** (`{row.slug}`)")
        add("")
        add(row.description)
        add("")
        add(f"Traces: {', '.join(f'`{t}`' for t in row.trace_ids)}")
        add("")

    # -- 3. hand-written analysis, chosen target, prediction -------------
    writeup = repo_root / "eval" / "week5_writeup.md"
    if writeup.is_file():
        add(writeup.read_text(encoding="utf-8").strip())
        add("")
    ...
    add("---")
    add("")
    add("Raw artefacts: `results/week5_traces.json`, `results/week5_traces/*.md` (one file per trace),")
    add("`eval/week5_questions.yaml` (the pool), `eval/week5_open_coding.yaml` (the notes).")
    add("")
    return "\n".join(lines)
